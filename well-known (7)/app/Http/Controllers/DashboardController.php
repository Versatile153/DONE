...
class DashboardController extends Controller
{
    ...
    public function ref(){
     ...
     $deposit1 = DB::table('deposits')->where('status', 'approved')->where('user_id', $user->id)->value('amount');
         ...
         return view('dashboard.referal')->with('deposits',$deposits)
         
         ->with('deposit',$deposit)->with('deposit1',$deposit1)
         ->with('earn',$earn)
         ->with('user',$user)
         ->with('user_ref',$user_ref)
         ->with('deposit4',$deposit4)
         ->with('withdrawals',$withdrawals);
    ...
    $total_mess=Message::all();
        ...
        return view('dashboard.admin')->with('users',$users)->with('total_user',$total_user)->with('total_dep',$total_dep)->with('total_with',$total_with);
...
return view('dashboard.admin_fund', ['transactions' => $transactions])->with('total_user',$total_user)
